...
def test_add_user(app, user_dao):
    """
    Test the add_user route
    """
    with app.test_client() as client:
        response = client.post('/add_user', json={'user_id': 1, 'username': 'admin', 'password': 'admin'})
        assert response.status_code == 201
        assert response.json == {'message': 'User created'}


def test_get_all_users(app, user_dao):
    """
...
def test_get_user(app, user_dao):
    ...
    with app.test_client() as client:
        ...
        client.get(f'/add_user', json={'user_id': user_id, 'username': 'user', 'password': 'user'})
...
def test_get_user_by_username(app, user_dao):
    """
    ...
    with app.test_client() as client:
        ...
        client.get(f'/add_user', json=json_string)
...
def test_update_user(app, user_dao):
    ...
    with app.test_client() as client:
        user_dao.add_user(User(1, 'admin', 'admin'))
        response = client.put('/update', json={'user_id': 1, 'username': 'admin', 'password': 'admin'})
        assert response.status_code == 200
        assert response.json == {'message': 'User updated'}


def test_add_book(app, book_dao):
    """
...
def test_get_all_books(app, book_dao):
    """
...
def test_update_book(app, book_dao):
    ...
    with app.test_client() as client:
        ...
        response = client.put('/updateBook', json={'id': 1, 'isbn': '123', 'title': 'Test Book', 'author': 'Author'})
        assert response.status_code == 200
        assert response.json == {'message': 'Book updated'}


def test_add_rented_book(app, rented_book_dao):
    """
...
def test_get_all_rented_books(app, rented_book_dao):
    """
...
def test_delete_book(app, book_dao):
    ...
    with app.test_client() as client:
        ...
        book_dao.add_book(Book(1, '123', 'Test Book', 'Author'))
        client.post('/add_book', json={'id': len(list_of_books) + 100, 'isbn': '123', 'title': 'Test Book', 'author': 'Author'})
        response = client.get('/books')
        print(response.json)
        last_object = response.json[-1]
